...
function _M.Order()
	...
	local client = GenericObjectPool:connection(OrderBeverageServiceClient, "order-beverage-service", 9042)
	ngx.say("after client:")
	...
	if not status then
    		...
    		if (ret.message) then
      			...
      			ngx.say("Failed to order here: " .. ret.message)
      			ngx.log(ngx.ERR, "Failed to order here: " .. ret.message)
    		else
      			ngx.header.content_type = "text/plain"
